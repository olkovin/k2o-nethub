...
:set listSrc ($listSrc . "    :if (([:len \$hs] > 0) and (\$hs != \"never\") and ([:find \$hs \"d\"] < 0) and ([:find \$hs \"h\"] < 0)) do={ :set st \"ONLINE\" }\r\n")
...
:set statusSrc ($statusSrc . ":foreach p in=\$peers do={ :local h [/interface wireguard peers get \$p last-handshake]; :if (([:len \$h]>0) and (\$h!=\"never\") and ([:find \$h \"d\"]<0) and ([:find \$h \"h\"]<0)) do={ :set on (\$on+1) } }\r\n")
