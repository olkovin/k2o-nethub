...
Якщо у вас є існуюча інсталяція MVSM (v3/v4), використовуйте скрипт оновлення:
...
Скрипт оновлення:
- Збереже всі існуючі peers, ключі та IP адреси
- Перейменує інтерфейс `wg_mvsm` → `wg_nethub`
- Оновить коментарі `MVSM` → `NETHUB`
- Встановить нові management скрипти v5.0
- Збереже ваші кастомні налаштування мережі/порту

**Переналаштування клієнтів не потрібне** — існуючі клієнти продовжать працювати.

## Файли

| Файл | Опис |
|------|------|
| `nethub_server_deploy.rsc` | Скрипт деплою хабу (нова інсталяція) |
| `nethub_upgrade.rsc` | Оновлення з MVSM v3/v4 до NETHUB v5.0 |
